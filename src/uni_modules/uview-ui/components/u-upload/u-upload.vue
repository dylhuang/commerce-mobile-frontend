    ...
    <view class="u-upload__wrap">
      <template v-if="previewImage">
        <view class="u-upload__wrap__preview" v-for="(item, index) in lists" :key="index">
          <image v-if="item.isImage || (item.type && item.type === 'image')" :src="item.thumb || item.url" :mode="imageMode" class="u-upload__wrap__preview__image" :style="{ width: $u.addUnit(btnSize), height: $u.addUnit(btnSize) }" @tap="onPreview(item, index)" />
          <block v-else-if="item.isVideo && item.cover">
            <image :src="item.cover" :mode="imageMode" class="u-upload__wrap__preview__image" :style="{ width: $u.addUnit(btnSize), height: $u.addUnit(btnSize) }" @tap="onPreview(item, index)" />
            <!--视频播放图标-->
            <image class="play_ico" :src="staticPicture.play_ico" @tap="onPreview(item, index)" />
          </block>

          <view v-else class="u-upload__wrap__preview__other" :style="{ width: $u.addUnit(btnSize), height: $u.addUnit(btnSize) }" @tap="onPreview(item, index)">
            <u-icon color="#80CBF9" size="26" :name="item.isVideo || (item.type && item.type === 'video') ? 'play-circle-fill' : 'folder'"></u-icon>
      ...
      <template v-if="isInCount && showAdd">
      ...
      <!-- 占位元素 -->
      <view v-if="isPlaceholderElement" :style="{ width: $u.addUnit(btnSize), height: 0 }"></view>
    </view>
  </view>
</template>
 ...
 * @property {Boolean}			previewFullImage	是否显示组件自带的预览功能（默认 true ）
  ...
  methods: {
    formatFileList() {
      ...
      const lists = fileList.map((item) => {
        ...
        // 判断item.url是否是blob地址
        if ((item.url && item.url.indexOf('blob:') === 0) || uni.$u.test.image(item.url || item.thumb)) {
          obj.isImage = true
        } else if (uni.$u.test.video(item.url || item.thumb)) {
          obj.isVideo = true
        }
        return obj
      })
    ...
    onPreview(item) {
      if (!this.previewFullImage || (!item.isImage && !item.isVideo)) return
      const canPreviewList = this.lists.filter((item) => item.isImage || item.isVideo)
      wx.previewMedia({
        sources: canPreviewList.map((res) => {
          return {
            url: res.url || res.thumb,
            type: res.isImage ? 'image' : 'video',
            poster: res.cover || ''
          }
        }),
        current: canPreviewList.findIndex((res) => res._id === item._id),
        fail() {
          uni.$u.toast('预览视频失败')
        }
      })
...
$u-upload-preview-border-radius: 4px !default;
$u-upload-preview-margin: 0 8px 8px 0 !default;
$u-upload-preview-margin-bottom: 8px !default;
$u-upload-image-width: 208rpx !default;
...
$u-upload-deletable-bgColor: rgba(0, 0, 0, 0.6) !default;
...
.u-upload {
  ...
  &__wrap {
    ...
    &__preview {
      ...
      margin-bottom: $u-upload-preview-margin-bottom;
      position: relative;
      overflow: hidden;
      @include flex;
