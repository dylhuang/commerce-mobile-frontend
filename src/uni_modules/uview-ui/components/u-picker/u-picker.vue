			...
			>
				...
				>
					<view
						...
						class="column-content u-picker__view__column__item u-line-1"
					...
					>{{ getItemText(item1) }}
					<text class="subTitle" v-if="item1.province&&item1.city&&item1.area">{{item1.province}}{{item1.city}}{{item1.area}}</text>
					</view>
				</picker-view-column>
			</picker-view>
...
export default {
	...
	data() {
		return {
			...
			// 上一次的变化列索引
			columnIndex: 0,
      isPicking: false, // 用于加强选择的校验，外部调用确定的时候可以获取该值是否为true来判断用户的滚动行为是否结束
		}
	},
	watch: {
		// 监听默认索引的变化，重新设置对应的值
    ...
    pickEnd() {
      ...
      this.isPicking = false
		...
		setColumns(columns) {
			...
			console.log(this.innerColumns,'this.innerColumns');
		},
		// 获取各列选中值对应的索引
		getIndexs() {
