<template>
  <view class="u-radio" @tap.stop="wrapperClickHandler" :style="[radioStyle]" :class="[`u-radio-label--${parentData.iconPlacement}`, parentData.borderBottom && parentData.placement === 'column' && 'u-border-bottom']">
    <view class="u-radio__icon-wrap" @tap.stop="iconClickHandler" :class="iconClasses" :style="[iconWrapStyle]">
      <slot name="icon">
        <view class="radio-fill" :style="[radioFillStyle]"></view>
        <!--<u-icon
  ...
  computed: {
    ...
    radioFillStyle() {
      ...
      if (this.elDisabled) {
        ...
        style['background-color'] = this.checked ? this.elInactiveColor : 'transparent'
      } else {
        style['background-color'] = this.checked ? themeColor.$themeColor : 'transparent'
      }
      return style
    },
    elIconColor() {
    ...
    iconWrapStyle() {
      // radio的整体样式
      const style = {}
      style.backgroundColor = this.checked && !this.elDisabled ? 'transparent' : '#ffffff'
      if (this.elDisabled) {
        style.borderColor = this.elInactiveColor
      } else {
        style.borderColor = this.checked ? themeColor.$themeColor : this.elInactiveColor
      }
