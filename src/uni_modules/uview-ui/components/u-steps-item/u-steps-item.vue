	...
	<view class="u-steps-item" ref="u-steps-item" :class="[`u-steps-item--${parentData.direction}`]">
		...
		<view class="u-steps-item__wrapper"
			...
			<slot name="icon">
				...
				<view v-else :style="{
						backgroundColor: statusClass === 'process' ? parentData.activeColor : '#CCCCCC',
						borderColor: statusClass === 'process' ? parentData.activeColor : '#CCCCCC',
            height: statusClass === 'process' ? '46rpx' : '34rpx',
            width: statusClass === 'process'? '46rpx' : '34rpx',
              ...
              color: index == parentData.current ? '#05ACFF!important' : '#fff',
             
						}">{{ index + 1}}</text>
					<u-icon v-else :color="statusClass === 'error' ? 'error' : parentData.activeColor" size="14" bold
